        do j=Jstr,Jend
          do i=IstrU-1,Iend
            ...
            rrp= (u(i+2,j,k,nrhs)-u(i+1,j,k,nrhs)) 
#   ifdef MASKING
     &         *rmask(i+1,j)
#   endif             
            rr = (u(i+1,j,k,nrhs)-u(i  ,j,k,nrhs)) 
#   ifdef MASKING
     &         *rmask(i  ,j)
#   endif
            rrm= (u(i  ,j,k,nrhs)-u(i-1,j,k,nrhs))
#   ifdef MASKING
     &         *rmask(i-1,j)
#   endif
            cff1=( cff*(u(i+1,j,k,nrhs)+u(i,j,k,nrhs))
     &       -abs(cff)*rr )*0.5         
        ...
        do j=JstrV-1,Jend
          do i=Istr,Iend
            ...
            rrp= (v(i,j+2,k,nrhs)-v(i,j+1,k,nrhs)) 
#   ifdef MASKING
     &         *rmask(i,j+1)
#   endif
            rr = (v(i,j+1,k,nrhs)-v(i,j,k,nrhs)) 
#   ifdef MASKING
     &         *rmask(i,j  )
#   endif
            rrm= (v(i,j  ,k,nrhs)-v(i,j-1,k,nrhs))
#   ifdef MASKING
     &         *rmask(i,j-1)
#   endif                    
            cff1=( cff*(v(i,j,k,nrhs) + v(i,j+1,k,nrhs))
     &        -abs(cff)*rr )*0.5
     ...
     &           +  Hz(i-1,j,k)+Hz(i-1,j-1,k) ) )
            rrp= (u(i,j+1,k,nrhs)-u(i,j  ,k,nrhs)) 
#   ifdef MASKING
     &         *pmask(i,j+1)
#   endif
            rr = (u(i,j  ,k,nrhs)-u(i,j-1,k,nrhs)) 
#   ifdef MASKING
     &         *pmask(i,j  )
#   endif
            rrm= (u(i,j-1,k,nrhs)-u(i,j-2,k,nrhs))
#   ifdef MASKING
     &         *pmask(i,j-1)
#   endif
            cff1=( cff*(u(i,j,k,nrhs)+u(i,j-1,k,nrhs))
     &        -abs(cff)*rr )*0.5      
     ...
     &    /( 0.25*( Hz(i  ,j,k)+Hz(i  ,j-1,k)
     &           +  Hz(i-1,j,k)+Hz(i-1,j-1,k) ) )
            rrp=(v(i+1,j,k,nrhs)-v(i  ,j,k,nrhs))
#   ifdef MASKING
     &         *pmask(i+1,j)
#   endif
            rr =(v(i  ,j,k,nrhs)-v(i-1,j,k,nrhs))
#   ifdef MASKING
     &         *pmask(i  ,j)
#   endif
            rrm=(v(i-1,j,k,nrhs)-v(i-2,j,k,nrhs))
#   ifdef MASKING
     &         *pmask(i-1,j)
#   endif
            cff1=( cff*(v(i,j,k,nrhs)+v(i-1,j,k,nrhs))
     &       -abs(cff)*rr )*0.5
