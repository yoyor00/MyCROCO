...
#endif
#ifdef BLOOM
      call init_bloom(ierr)
#  ifdef DIAGNOSTICS_BIO
      call init_bloom_diag(ierr)
#  endif
#endif
#if defined BIOLink && defined BIOLink_PAR_eval && !defined DIAGNOSTICS_BIO
      call init_biolinkpar_history_fields()
#endif
#ifdef OBSTRUCTION
...
# endif
   ...
   !  init_bloom
   !---------------------------------------------------------------------
   subroutine init_bloom(ierr)
      use param, ONLY: stdout
      use croco_namelist, ONLY: bloomfilename
#if defined MPI
      use scalars, ONLY: mynode
#endif
      implicit none
      ...
      open (testunit, file=trim(bloomfilename), status='old', iostat=ios)
      if (ios == 0) then
         close (testunit)
      else
         MPI_master_only write (stdout, *) &
            'Error: cannot open bloom file ', trim(bloomfilename)
         ierr = ierr + 1
      end if

   end subroutine init_bloom

#  ifdef DIAGNOSTICS_BIO
   !---------------------------------------------------------------------
   !  init_bloom_diag
   !  Check bloomdiagfilename is accessible and wire it to filevardiag.
   !---------------------------------------------------------------------
   subroutine init_bloom_diag(ierr)
      use param, ONLY: stdout
      use croco_namelist, ONLY: bloomdiagfilename
      use comBIOLink, ONLY: filevardiag
#if defined MPI
      use scalars, ONLY: mynode
#endif
      implicit none
      ...
      filevardiag = bloomdiagfilename
      open (testunit, file=trim(bloomdiagfilename), status='old', iostat=ios)
      if (ios == 0) then
         close (testunit)
      else
         MPI_master_only write (stdout, *) &
            'Error: cannot open bloom diagnostics file ', trim(bloomdiagfilename)
         ierr = ierr + 1
      end if

   end subroutine init_bloom_diag
#  endif
#endif

#if defined BIOLink && defined BIOLink_PAR_eval && !defined DIAGNOSTICS_BIO
   !---------------------------------------------------------------------
   !  init_biolinkpar_history_fields
   !  Wire out_his_Biolinkpar into the legacy wrthis array.
   !---------------------------------------------------------------------
   subroutine init_biolinkpar_history_fields()
      use ncscrum, ONLY: wrthis, indxTime, indxPAR
      use croco_namelist, ONLY: out_his_Biolinkpar
      implicit none

      wrthis(indxPAR) = out_his_Biolinkpar
      if (out_his_Biolinkpar) wrthis(indxTime) = .true.

   end subroutine init_biolinkpar_history_fields
#endif
...
#  endif
# else /* BLOOM */
      use param, ONLY: NumBLMdiag3d, NumBLMdiag2d, NumBLMdiag1d
      use ncscrum, ONLY: wrtdiabio3d, wrtdiabio2d, wrtdiabio1d
# endif /* !BLOOM */
      implicit none
      integer :: iflux
# ifndef BLOOM
...
#  endif
# else /* BLOOM */
      do iflux = 1, NumBLMdiag3d
         wrtdiabio3d(iflux) = .true.
      end do
      wrtdiabio3d(NumBLMdiag3d + 1) = .true.
      do iflux = 1, NumBLMdiag2d
         wrtdiabio2d(iflux) = .true.
      end do
      wrtdiabio2d(NumBLMdiag2d + 1) = .true.
# endif /* !BLOOM */
