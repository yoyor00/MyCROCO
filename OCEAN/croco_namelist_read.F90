...
#if defined DIAGNOSTICS_BIO && !defined BLOOM
...
#endif
#ifdef BLOOM
      namelist /croco_bloom/ bloomfilename
#  ifdef DIAGNOSTICS_BIO
      namelist /croco_bloom_diag/ bloomdiagfilename
#  endif
#endif
#if defined BIOLink && defined BIOLink_PAR_eval && !defined DIAGNOSTICS_BIO
      namelist /croco_BIOLink_par_history_fields/ out_his_Biolinkpar
#endif
#ifdef OBSTRUCTION
...
#ifdef BLOOM
      ! --- croco_bloom (optional) ---
      call check_nml_presence(nmlunit, "croco_bloom", .false., found, ierr)
      if (found) then
         read (nmlunit, nml=croco_bloom, iostat=ios); rewind (nmlunit)
         if (ios /= 0) then
            call fatal_nml_error("croco_bloom (parse error)")
            ierr = ierr + 1; close (nmlunit); return
         end if
      end if
#  ifdef DIAGNOSTICS_BIO
      ! --- croco_bloom_diag (optional) ---
      call check_nml_presence(nmlunit, "croco_bloom_diag", .false., found, ierr)
      if (found) then
         read (nmlunit, nml=croco_bloom_diag, iostat=ios); rewind (nmlunit)
         if (ios /= 0) then
            call fatal_nml_error("croco_bloom_diag (parse error)")
            ierr = ierr + 1; close (nmlunit); return
         end if
      end if
#  endif
#endif
#if defined BIOLink && defined BIOLink_PAR_eval && !defined DIAGNOSTICS_BIO
      ! --- croco_BIOLink_par_history_fields (optional) ---
      call check_nml_presence(nmlunit, "croco_BIOLink_par_history_fields", .false., found, ierr)
      if (found) then
         read (nmlunit, nml=croco_BIOLink_par_history_fields, iostat=ios); rewind (nmlunit)
         if (ios /= 0) then
            call fatal_nml_error("croco_BIOLink_par_history_fields (parse error)")
            ierr = ierr + 1; close (nmlunit); return
         end if
      end if
#endif
...
# if defined PSOURCE || defined PSOURCE_MASS || defined PSOURCE_NCFILE
      ...
      if (psource_Nsrc > 0) then
         if (.not. allocated(psource_Isrc)) then
            allocate (psource_Isrc(psource_Nsrc)); psource_Isrc = 1
         end if
         if (.not. allocated(psource_Jsrc)) then
            allocate (psource_Jsrc(psource_Nsrc)); psource_Jsrc = 1
         end if
         ...
         if (.not. allocated(psource_qbardir)) then
            allocate (psource_qbardir(psource_Nsrc)); psource_qbardir = 1.0
         end if
#  if defined TRACERS
...
#endif
#ifdef BLOOM
      MPI_master_only WRITE (stdout, nml=croco_bloom)
#  ifdef DIAGNOSTICS_BIO
      MPI_master_only WRITE (stdout, nml=croco_bloom_diag)
#  endif
#endif
#if defined BIOLink && defined BIOLink_PAR_eval && !defined DIAGNOSTICS_BIO
      MPI_master_only WRITE (stdout, nml=croco_BIOLink_par_history_fields)
#endif
#ifdef OBSTRUCTION
...
#endif
#if defined DIAGNOSTICS_BIO && !defined BLOOM && defined AVERAGES
