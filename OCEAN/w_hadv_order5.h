...
!$acc loop independent
          DO j = Jstr,Jend+1  ! j_loop_y_flux_5
            DO i = Istr,Iend
                                        !
              IF ( j.eq.jmin-2 ) THEN   ! 2nd order flux next to south boundary
                                        !
...
#  endif
                                            !
              ELSE IF ( j.eq.jmax+2 ) THEN  ! 2nd order flux next to north boundary
                                            !
                WFe(i,j) = Hvom_w(i,j,k)*FLUX2(
     &             wz(i,j-1,k,nrhs), wz(i,j,k,nrhs), Hvom_w(i,j,k), cdif)
                                            !
...
#  endif
                                            !
              ELSE IF ( i.eq.imax+2 ) THEN  ! 2nd order flux next to north boundary
                                            !
                WFx(i,j) = Huon_w(i,j,k)*FLUX2(
     &             wz(i-1,j,k,nrhs), wz(i,j,k,nrhs), Huon_w(i,j,k), cdif)
