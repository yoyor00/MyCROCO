...
- `BIOLink` — BIOLink coupler between CROCO and an external biogeochemical model
  (exactly one bio model must also be defined)
  - `BLOOM` — BLOOM multi-species phytoplankton model
    - `key_benthos` — Benthic filter-feeder module within BLOOM
    - `key_oyster` — Oyster DEB model (requires `BLOOM`)
  - `ECO3M` — ECO3M biogeochemical model
    - `DYFAMED` — ECO3M DYFAMED test-case initialisation
    - `PHYBIO_2ways` — Two-way coupling of solar shortwave between physics and ECO3M
  - `PEPTIC` — PEPTIC biogeochemical model
  - `METeOR` — METeOR biogeochemical model
  - `BIOLink_PAR_eval` — Compute and output photosynthetically active radiation (PAR);
    compatible with any BIOLink bio model; PAR goes to the history file unless
    `BLOOM && DIAGNOSTICS_BIO`, in which case it goes to the separate diag file instead
  - `BIOLink_UPDATE_CONCBIO` — Update biological concentrations from BIOLink to CROCO tracers
  - `BIOLink_verif_conserv` — Enable mass-conservation verification diagnostics
