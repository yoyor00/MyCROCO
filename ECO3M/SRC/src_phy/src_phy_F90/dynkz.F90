...
!***********************************************************************
       ...
       Use comrunmod
       Use comdynmod
       dimension ZY(jpzt),ZZ(jpzt)
!
! Calcul de la densite potentielle, Brunt Vaissalla, cisaillement
     ...
     &      DE3W(jk))**2
       enddo
!
! Calcul des longueurs de melange (eq. de 2ieme degre)
!
       do jk=2,nzt-1
...
! MB : coefficient de turbulence pour la chaleur = AVM * Prandtl :
         AVT(jk)=xpdl*AVM(jk)
         AVE(jk)=fave*AVM(jk)
       enddo
       AVT(nzt)=0.
