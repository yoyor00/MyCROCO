...
CC CHANGE PRESSURE FROM INPUT UNITS OF DECIBARS TO BARS
C     barrier.n
C     conversion of C in K
      DP=DBLE(P)
      DT=DBLE(T)
      DS=DBLE(S)
      DP=DP/10.D0
C SQUARE ROOT SALINITY.
      SR = DSQRT(DABS(DS))
C DENSITY PURE WATER AT ATM PRESS IN KG/M3 = (10**-3)GM/CM3
...
CC DENSITY - 1000 AT ATMOSPHERIC PRESSURE
      IF(ABS(P).GE..00001)GO TO 1
CC SIGMA REMAINS UNCHANGED, SINCE IS (10**-3)GM/CM3
      RETURN       
...
C
     ...
     X+148.4206D0)*DT+19652.21D0
      K0 = (B1*SR + A1)*DS + KW
C EVALUATE PRESSURE POLYNOMIAL AND RETURN
      K = (B*DP + A)*DP + K0
      SIGMA0 = SNGL((K*DBLE(SIGMA0) + 1000.D0*DP)/(K-DP))
CC SIGMA REMAINS UNCHANGED SINCE IS (10**-3)GM/CM**3
      RETURN
      END
