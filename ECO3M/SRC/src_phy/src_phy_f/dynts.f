...
C***********************************************************************
       subroutine DYNTS
C
C Integration de Temperature et Salinite
C
       Use comrunmod
       Use comdynmod
       dimension ZWORK(jpzt)
C
C Initialisation - Rappel sur colonne
       call DYNRAPPEL
       do jk=1,nzt-1
...
C
C Rappel en surface 
C       if (rapfut.eq.0.) goto 501
       call DYNRAPSURF
C Rappel donnee future
...
C Rappel donnee passee
C 501   if (rappas.eq.0.) goto 502
...
C Surface
       DZW(1)=0.
       DZX(1)=dun-DZY(1)
C MB: dans dynfluxsur.f:   q=(npasflux0*zq1+(npasflux-npasflux0)*zq0)/npasflux
...
C Champ after
       ...
       enddo
C Filtre temporel et swap des tableaux
       do jk=1,nzt
