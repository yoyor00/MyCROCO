Implicit none 
Integer:: err,nb,i,j
Character(100) :: fichpar, fichconf, fichout
Character(25) :: chain1,chain2
Character(120) :: chain
Character(50) :: chaine
real(8) :: num
...
Write(*,*)" A LIRE ---- A LIRE --- A LIRE --- A LIRE --- A LIRE --- A LIRE ----"
Write(*,*)"  ---- TAPER 'ENTREE' A LA FIN DE LA LECTURE                        "
Write(*,*)" -------------------------------------------------------------------"
...
! barrier.n: possibility of a variable config filename
write(*,*)'nom du fichier de configuration (config.ini_txt par exemple) ?'
read(*,*) fichconf
...
! barrier.n: name of the output file has been changed
fichout =  trim(fichconf) // "_conv"
...
Inquire(file=fichconf,Exist=Present)  ! barrier.n
if (.not. present) then
    write(*,*) 'le fichier', trim(fichconf), ' n existe pas, ce programme va être arreté '
    stop
endif

write(10,"(3A)") 'cp ', trim(fichconf), ' fc'
...
! barrier.n: change in the cp output file
write(10,"(2A)") 'cp fc ', fichout
close(10)

call SYSTEM('chmod 777 txt2nb.sh')
call SYSTEM('./txt2nb.sh')
call SYSTEM('rm fc')
call SYSTEM('rm fc.bak')
call SYSTEM('rm txt2nb.sh')
end
