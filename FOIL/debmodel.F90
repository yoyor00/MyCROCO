...
CONTAINS
   ...
   SUBROUTINE deb_init(restart)
      ...
      !&E              and read constants for species
      !&E
      !&E ** Description    :
      ...
      DO n = 1, patches%nb
         ...
         DO m = 1, nb_part_nc
            ...
            END IF ! ce if commenté chez clara
            ...
            IF (patch%species == 'anchovy') THEN
               ...
               Wat = exp(-3.19_rsh + 0.38_rsh*size + 0.586_rsh*log(NRJ_g/1000.0_rsh) &
                         - 0.036_rsh*size*log(NRJ_g/1000.0_rsh))
               ! Gatti et al. 2017
               Wash = exp(-2.9326_rsh + 0.9125_rsh*log(Wat))
               ! Wash=f(Water)
            END IF !( species == 'anchovy')

            IF (patch%species == 'sardine') THEN
               ! Water - Conversion avec relation poids eau vs. taille * Energie
               Wat = exp((-0.841256_rsh) + 0.199038_rsh*size + 0.277582_rsh*log(NRJ_g/1000.0_rsh) &
                         - 0.008299_rsh*size*log(NRJ_g/1d3))
               Wash = exp(-2.9325954_rsh + 0.912532_rsh*log(Wat))          ! Wash=f(water)
            END IF !( species == 'sardine')
   ...
   SUBROUTINE deb_cycle(particle, dt, year, month, day, species)
      ...
      IF (H >= Hp) THEN
         ...
         IF (species == 'sardine') THEN
            ...
            END IF
         END IF ! if (species == 'sardine')

         IF (pG < 0.0_rsh) THEN     !Bad condition, need to remobilise energy
            pM2 = min(-pG, R*86400.0_rsh/dt)
            IF (particle%season .and. Gam > 0.0_rsh) pM3 = min(Gam*86400.0_rsh/dt, (-pG - pM2)/Kr)
            ! J/d, second mobilization from gametes
         END IF
      ...
      ELSE
         ...
         H = Hp
         IF (particle%season) THEN
            IF (Gam > Eb*2.0_rsh) THEN
               ! enough energy to spawn 1 batch and same amount remains for vitellogenesis (Somarakis Reproduce Del.), explicit
               Ebatch = Eb
               particle%Nbatch = particle%Nbatch + 1
               particle%Neggs = particle%Neggs + Ebatch/E0*(particle%super/2.0_rlg)
               ! total egg spawned per particle over dt_spawn (see ibm)
               particle%Neggs_tot = particle%Neggs_tot + Ebatch/E0*(particle%super/2.0_rlg)
               ! total egg spawned per particle over life cycle
            ...
            IF (lastbatch) THEN
               ...
               IF (species == 'anchovy') THEN
                  particle%Neggs = particle%Neggs + Ebatchlast/E0*(particle%super/2.0_rlg)
                  ! total egg spawned per particle over dt_spawn (see ibm)
               END IF
               IF (month <= 7 .and. species == 'sardine') THEN
                  ...
                  ! total egg spawned per particle over dt_spawn (see ibm)
               END IF
               particle%Neggs_tot = particle%Neggs_tot + Ebatchlast/E0*(particle%super/2.0_rlg)
               ! total egg spawned per particle over life cycle
            END IF
         ...
         ELSE
            dR = pR*Ker - pM2
            ! only necessary fraction is mobilized for emergency, pR can be negative,
            ! then energy goes from R to maturity maintenance (like pM2)
         END IF   ! if (particle%season)
      END IF       ! if (H<Hp)
      ...
      IF (species == 'anchovy') THEN
         ! Water - Conversion avec relation poids eau vs. taille * Energie
         Wat = exp(-3.19_rsh + 0.38_rsh*size + 0.586_rsh*log(NRJ_g/1000.0_rsh) - 0.036_rsh*size*log(NRJ_g/1000.0_rsh))
         ! Gatti et al. 2017
         Wash = exp(-2.9326_rsh + 0.9125_rsh*log(Wat))
         ! Wash=f(Water)
      END IF !( species == 'anchovy')
   ...
   FUNCTION get_Xdeb(xpos, ypos, spos, size, minprof, maxprof, debug)
      ...
      !    maxwidth=min(maxwidth,3.0_rsh)
      !    ! ordre de grandeur du max proie observe, ~1cm en longueur,
      !    ! ~ 3mm en largeur. Coherent avec representation  taille modele NPZD, cf routine ci-dessous
      !
      !    IF (species == 'anchovy') THEN
