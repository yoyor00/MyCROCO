...
!
      ...
      parameter (NP_XI=14,  NP_ETA=2,  NNODES=NP_XI*NP_ETA)
      parameter (NPP=1)
...
/* ntrc_subs : number of advected substances (not fixed, neither benthic) */
      ...
      ! Je déclare les deux compteurs pour les substances
      ! (G. Koenig, 26/11/2021)
      integer itsubs1,itsubs2
/* J'ajoute les clefs pour MUSTANG (G.Koenig, 24/11/2021) */
#  ifdef MUSTANG
      INTEGER,PARAMETER :: ntrc_sedim=3
#  else
      INTEGER,PARAMETER :: ntrc_sedim=0
#  endif
!  Je rajoute la déclaration pour BIOlink et BLOOM
!  (G. Koenig, 17/11/2021)
#  if defined BIOLink
!   !!!!!!!!!!!!!!!!!!! ntrc_biol : number of advected BIO substances !!!!!
#   if defined BLOOM
!     WARNING : depends on CPPkeys (modules bio add variables)  
!     evaluation of ntrc_biol and ntfix
#     include "param_BLOOM.h" 
#   else /* Si il n'y a pas d'autre package que BLOOM (G. Koenig, 24/11/2021)*/
      INTEGER,PARAMETER :: ntrc_biol=4
#   endif /*BLOOM*/
#  else /* S'il n'y a pas BIOLink (G.Koenig, 24/11/2021)*/
      INTEGER,PARAMETER :: ntrc_biol=0, ntfix=0
#  endif /*BIOLink*/
/* Je peux calculer les indices pour les variables dans substance */
      parameter (ntrc_subs=ntrc_biol+ntrc_sedim , ntrc_substot=ntrc_subs+ntfix )
      parameter (itsubs1 = itemp+ntrc_salt+1)
      parameter (itsubs2=itemp+ntrc_salt+ntrc_subs)
# else /* S'il n'y a pas Substance */
...
# elif defined MUSTANG
      parameter (NT=ntrc_temp+ntrc_salt+ntrc_pas
  &                         +ntrc_bio+ntrc_sed+2*ntrc_subs)
      parameter (NTot=NT+ntfix)

# else
      parameter (NT=ntrc_temp+ntrc_salt+ntrc_pas+ntrc_bio+ntrc_sed)
      parameter (NTot=NT)
# endif /* SUBSTANCE */
