...
!======================================================================
!
! param.h - MS3DVAR FILTER Variant Grid Parameters
! =================================================
!
! This is the parameter file for the MS3DVAR FILTER variant.
! It includes the minimal param_ms3dvar.h and can add FILTER-specific settings.
!
...
!   - Add FILTER variant-specific overrides below (if any)
!
...
! - Filter-specific grid parameters
! - Tiling configuration (NSUB_XF, NSUB_EF)
...
!----------------------------------------------------------------------
! FILTER VARIANT-SPECIFIC PARAMETER OVERRIDES
!
! If the FILTER variant needs different grid dimensions or tiling
...
! - nratio=1 (no downsampling for FILTER)
! - NSUB_XF=6, NSUB_EF=6 (6x6 tiling)
!
! To override, uncomment and modify below:

! Example: Different high-resolution grid for FILTER
!     parameter (LLmH=xxx,  MMmH=yyy, NnH=zz)

! Example: Different downsampling for FILTER
!     parameter (nratio=2, nhalf=nratio/2+1)

! Example: Different tiling for FILTER
!     parameter (NSUB_XF=4, NSUB_EF=4, NPPF=16)

!----------------------------------------------------------------------
! ADDITIONAL CROCO PARAMETERS (if needed)
!
! param_ms3dvar.h provides the minimal set of CROCO parameters.
! If you need additional CROCO-specific parameters for your FILTER variant,
! define them here.
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
! END OF FILTER VARIANT PARAMETERS
!----------------------------------------------------------------------
