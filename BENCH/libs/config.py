...
class Config:
    ...
    def parse(self) -> None:
        ...
        parser.add_argument('-s', '--steps', help="List of steps to run.", dest='modes', default='build,run,check,plot')
        parser.add_argument('-R', '--runs', help="Number or runs to perform.", default='1')
        ...
        parser.add_argument(      '--continue', help="Continue instead of stopping on first error.", dest='continue_on_error', action='store_true')
        parser.add_argument(      '--report', help="Build a status report.", action='store_true')
    ...
    def apply_meta(self):
        ...
        self.variant_names = self.apply_wildcard_enabling(self.variant_names, avail_variants)
