    ...
    if [ ${USE_OCE} -eq 1 ] ; then
	    ...
	    [[ ${AGRIFZ} > 0 ]] && { mm="_${nn}" ;} || { mm="" ;}
            ...
            if [[ ${CPL_restart} == "TRUE" ]] && [[ ! -z ${oce_rst_file} ]]; then
                ...
                . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_preexisting_output_files.sh "${oce_rst_file}${agrif_ext}" oce.nc${agrif_ext} croco ${mm}
            else
                echo 'create restart file for oasis from calm conditions for variables:'${varlist}
                . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_calm_conditions.sh ${oce_rst_file}${agrif_ext} oce.nc${agrif_ext} croco "${varlist}"
            fi            
        done
    fi
#
    if [ ${USE_WAV} -eq 1 ] ; then
        ...
        if [[ ${CPL_restart} == "TRUE" ]] && [[ ! -z ${wav_rst_file} ]]; then
            ...
            . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_preexisting_output_files.sh "${wav_rst_file}" wav.nc ww3
        else
            echo 'create restart file for oasis from calm conditions for variables:'${varlist}
            [[ ! -f ${wav_rst_file} ]] && { echo "${wav_rst_file} is not there to create oasis restart file, we stop..." ; exit ;}
            . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_calm_conditions.sh ${wav_rst_file} wav.nc ww3 "${varlist}"
        fi
    fi
#
