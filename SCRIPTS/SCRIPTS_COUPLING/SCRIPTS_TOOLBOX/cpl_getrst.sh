    ...
    if [ ${USE_ATM} -eq 1 ] ; then
        ...
        for domatm in $wrfcpldom ; do
            ...
            if [[ ${CPL_restart} == "TRUE" ]] && [[ ! -z ${atm_rst_file} ]]; then
                printf "Create atmosphere restart file for oasis from preexisting file ${atm_rst_file}" #|tee cpl_getrst.log   
                printf "\n  -> See log file ${EXEDIR}/cpl_getrst.log\n"

                for ocedom in $loopoce; do
                    domoce="d0$(( ${ocedom} + 1 ))"
                    if [ ${domatm} == "d01" ]; then
                        . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_preexisting_output_files.sh "${atm_rst_file}" atm.nc wrf "WRF_${domatm}_EXT_${domoce}" >> cpl_getrst.log
                    else
                        . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_preexisting_output_files.sh "${atm_rst_file}" atm${domatm}.nc wrf "WRF_${domatm}_EXT_${domoce}" >> cpl_getrst.log
                    fi
                done
                ...
                if [ ${domatm} == "d01" ]; then
                    . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_calm_conditions.sh wrfinput_${domatm} atm.nc wrf "${varlist}"  >> cpl_getrst.log
                else 
                    . ${SCRIPTDIR}/OASIS_SCRIPTS/create_oasis_restart_from_calm_conditions.sh wrfinput_${domatm} atm${domatm}.nc wrf "${varlist}"  >> cpl_getrst.log
                fi
            fi
        done
    fi
...
#
    if [ ${USE_WAV} -eq 1 ] ; then
        ...
        if [[ ${CPL_restart} == "TRUE" ]] && [[ ! -z ${wav_rst_file} ]]; then
            ...
            printf "Create restart file for oasis from calm conditions for variables: ${varlist}" #|tee cpl_getrst.log
            printf "\n  -> See log file ${EXEDIR}/cpl_getrst.log\n"
            [[ ! -f ${wav_rst_file} ]] && { echo "${wav_rst_file} is not there to create oasis restart file, we stop..." ; exit 1 ;}
