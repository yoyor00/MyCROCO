...
if [[ ${models[@]} =~ "cpl" ]] || [[ ${models[@]} =~ "wav" ]] || [[ ${models[@]} =~ "atm" ]] || [[ ${models[@]} =~ "toy" ]] ; then
    ...
    echo '-----------------------------'
    [ -d $MY_CONFIG_HOME/SCRIPT_CPL ] && \rm -Rf $MY_CONFIG_HOME/SCRIPT_CPL
    cp -Rf $TOOLS_DIR/Coupling_tools/script_run_cpl_inter $MY_CONFIG_HOME/SCRIPT_CPL

    # Edit myjob.sh to add CPU lines for each model
    cd $MY_CONFIG_HOME/SCRIPT_CPL
    ...
    cd $MY_CONFIG_HOME/SCRIPT_CPL/ROUTINES/PATHS
    ...
    # Create the env file
    [ -d ${MY_CONFIG_HOME}/SCRIPT_CPL/ROUTINES/MACHINE/${MACHINE} ] && cd ${MY_CONFIG_HOME}/SCRIPT/ROUTINES/MACHINE/${MACHINE} || { echo "No environement for ${MACHINE} in ${MY_CONFIG_HOME}/SCRIPT_CPL/ROUTINES/MACHINE/${MACHINE}"; exit ;}
    ...
    cd SCRIPT_CPL/ROUTINES/NAMELISTS
